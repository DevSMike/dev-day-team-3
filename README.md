...
<a name="прим"></a>
### Примечение
После выполнения заданий и завершения работы сервера с хакатоном был произведен рефакторинг кода. Все тесты выполняелись локально в классе **HttpClientLocalController**. 

В данном классе была воссоздана логика сервера, который отвечает на запрос с выполненным заданием. Тестовые запросы отправлялись с использованием postman. Тесты, написанные для проверки работы программы можно найти в пакете *postman*. 
